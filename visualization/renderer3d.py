...
class Renderer3D:
    ...
    def _add_domain_box(self, xy_extent: float, gap_distance: float) -> None:
        """Dibuja una caja gris semitransparente tipo wireframe y los electrodos de cátodo y ánodo.
        ...
        if self._cathode_actor is not None:
            self._plotter.remove_actor(self._cathode_actor)
        if self._anode_actor is not None:
            self._plotter.remove_actor(self._anode_actor)
        ...
        # Cátodo (Placa inferior en Z = 0) - Placa metálica plateada
        cathode_plane = self._pv.Plane(
            center=(0.0, 0.0, 0.0),
            direction=(0.0, 0.0, 1.0),
            i_size=2.0 * xy_extent,
            j_size=2.0 * xy_extent,
        )
        self._cathode_actor = self._plotter.add_mesh(
            cathode_plane,
            color="silver",
            opacity=0.4,
            show_edges=True,
        ...
        # Ánodo (Placa superior en Z = gap_distance) - Placa metálica cobriza/dorada
        anode_plane = self._pv.Plane(
            center=(0.0, 0.0, gap_distance),
            direction=(0.0, 0.0, 1.0),
            i_size=2.0 * xy_extent,
            j_size=2.0 * xy_extent,
        )
        self._anode_actor = self._plotter.add_mesh(
            anode_plane,
            color="goldenrod",
            opacity=0.4,
            show_edges=True,
    ...
    def clear(self) -> None:
        ...
        if self._ion_actor is not None:
