...
class Renderer3D:
    ...
    def __init__(self, parent=None) -> None:
        ...
        # Electrode accumulator actors
        self._anode_e_mesh = None
        self._anode_e_actor = None
        self._cathode_i_mesh = None
        self._cathode_i_actor = None
        ...
        # Electrode accumulator actors (start empty)
        self._anode_e_mesh = None
        self._anode_e_actor = None
        self._cathode_i_mesh = None
        self._cathode_i_actor = None
    ...
    def update_particles(self, electron_positions, neutral_positions=None, ion_positions=None,
                         recombined_positions=None, anode_electrons=None, cathode_ions=None) -> None:
        ...
        # 5. Deposited on anode (Cyan)
        if anode_electrons is None or len(anode_electrons) == 0:
            anode_e_points = np.empty((0, 3))
        else:
            anode_e_points = np.asarray(anode_electrons, dtype=float)

        # 6. Deposited on cathode (Lime)
        if cathode_ions is None or len(cathode_ions) == 0:
            cathode_i_points = np.empty((0, 3))
        else:
            cathode_i_points = np.asarray(cathode_ions, dtype=float)
        ...
        self._replace_actor("anode_e",   anode_e_points,   "cyan",       9)
        self._replace_actor("cathode_i", cathode_i_points, "limegreen",  9)
    ...
    def _replace_actor(self, kind: str, points: np.ndarray, color: str, size: int) -> None:
        ...
        if not hasattr(self, actor_attr):
            print(f"Error en Renderer3D: El atributo {actor_attr} no existe en la clase.")
            return
