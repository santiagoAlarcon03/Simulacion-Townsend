...
class SimulationEngine:
    ...
    def __init__(self, config: SimulationConfig) -> None:
        ...
        # Inicializa el campo eléctrico vectorial apuntando en el eje Z
...
# --------------------------------------------------
        ...
        # Aplica condiciones de frontera (rebote/reflexión en las paredes del cubo)
        # Aplica condiciones de frontera (rebote/reflexión en las paredes del cubo)
        ...
        # Los iones positivos también permanecen dentro del dominio.
        self._reflect_into_domain(state.ion_positions,state.ion_velocities,)

        # Re-verifica si tras las reflexiones o pérdidas quedan electrones en el dominio
        if state.positions.size == 0:
            state.time += dt
        ...
        if ionize_mask.any() and state.positions.shape[0] < self.config.max_particles:
            ...
            ion_velocities = particles.random_thermal_velocities(
                new_count,
                self.rng,
                self.config.gas_temperature * 0.05,
            )
